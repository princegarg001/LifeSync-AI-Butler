  ...
  Future<List<Insight>> generateInsights(Session session, int userId) async {
    ...
    if (overdueTasks.isNotEmpty) {
      insights.add(Insight(
        ...
        type: 'warning',
        title: 'Overdue Tasks',
        message: 'You have ${overdueTasks.length} overdue task${overdueTasks.length > 1 ? 's' : ''}. Consider rescheduling or completing them.',
        priority: 2,
    ...
    if (urgentToday.isNotEmpty) {
      insights.add(Insight(
        ...
        message: '${urgentToday.length} high-priority task${urgentToday.length > 1 ? 's' : ''} due today. Focus on these first!',
        priority: 2,
    ...
    if (hour >= 9 && hour <= 11) {
      insights.add(Insight(
        ...
        message: 'Morning hours are great for focused work. Tackle your most challenging task now!',
        priority: 1,
      ...
      insights.add(Insight(
        ...
        message: 'Energy typically dips after lunch. Consider a quick walk or tackling easier tasks.',
        priority: 0,
        iconName: 'coffee',
        isDismissed: false,
        generatedAt: now,
